...
: run this script from xpct/tests/methodtest/wiremock dir

if test -e "wiremock-standalone-2.6.0.jar";then
	echo "existing"
else
	echo "not existing"
curl -H "Accept: application/zip" http://repo1.maven.org/maven2/com/github/tomakehurst/wiremock-standalone/2.6.0/wiremock-standalone-2.6.0.jar -o wiremock-standalone-2.6.0.jar	
fi
